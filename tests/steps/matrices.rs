...
use lab_raytracing_rs::matrices::{Matrix2x2, Matrix3x3, Matrix4x4, identity_matrix};
...
pub fn steps() -> Steps<MyWorld> {
    ...
    steps.given_regex(
        ...
        |mut world, ctx| {
            ...
            world.matrices.insert(name, Matrix::Matrix2x2(m));
    ...
    steps.given_regex(
        ...
        |mut world, ctx| {
            ...
            world.matrices.insert(name, Matrix::Matrix3x3(m));
    ...
    steps.given_regex(
        ...
        |mut world, ctx| {
            ...
            world.matrices.insert(name, Matrix::Matrix4x4(m));
    ...
    steps.then_regex(
        ...
        |world, ctx| {
            ...
            let v = match m {
                Matrix::Matrix2x2(m) => m.at(w, h),
                Matrix::Matrix3x3(m) => m.at(w, h),
                Matrix::Matrix4x4(m) => m.at(w, h),
            };
    ...
    steps.then_regex(
        ...
        |world, ctx| {
            ...
            match (m1, m2) {
                (Matrix::Matrix2x2(m1), Matrix::Matrix2x2(m2)) => assert!(m1 == m2),
                (Matrix::Matrix3x3(m1), Matrix::Matrix3x3(m2)) => assert!(m1 == m2),
                (Matrix::Matrix4x4(m1), Matrix::Matrix4x4(m2)) => assert!(m1 == m2),
    ...
    steps.given_regex(
        ...
        |mut world, ctx| {
            ...
            world.matrices.insert(name3, Matrix::Matrix4x4(matrix));
    ...
    steps.given_regex(
        ...
        |mut world, ctx| {
            ...
            let matrix = match world.matrices.get(&name2).unwrap() {
                Matrix::Matrix4x4(m) => m.invert().unwrap(),
                _ => panic!("matrix needs to be in 4x4 form")
            };

            world.matrices.insert(name1, Matrix::Matrix4x4(matrix));
    ...
    steps.given_regex(
        ...
        |mut world, ctx| {
            ...
            let matrix = match world.matrices.get(&matrix_name).unwrap() {
                Matrix::Matrix4x4(m) => Matrix::Matrix3x3(m.submatrix(w, h)),
                Matrix::Matrix3x3(m) => Matrix::Matrix2x2(m.submatrix(w, h)),
    ...
    steps.then_regex(
        ...
        |world, ctx| {
            ...
            let minor = match world.matrices.get(&matrix_name).unwrap() {
                Matrix::Matrix3x3(m) => m.minor(w, h),
                _ => panic!("matrix needs to be in 3x3 form")
            };

            assert_eq!(desired_minor, minor);
    ...
    steps.then_regex(
        ...
        |world, ctx| {
            ...
            let desired_minor = ctx.matches[4].parse::<f64>().unwrap();
            let minor = match world.matrices.get(&matrix_name).unwrap() {
                Matrix::Matrix3x3(m) => m.cofactor(w, h),
                Matrix::Matrix4x4(m) => m.cofactor(w, h),
    ...
    steps.then_regex(
        ...
        |world, ctx| {
            ...
            assert_eq!(desired_determinant, determinant);
    ...
    steps.then_regex(
        r#"^([A-Z]+) is not invertible$"#,
        |world, ctx| {
            let matrix_name = ctx.matches[1].clone();
            let invertible = match world.matrices.get(&matrix_name).unwrap() {
                Matrix::Matrix2x2(m) => m.invertible(),
                Matrix::Matrix3x3(m) => m.invertible(),
                Matrix::Matrix4x4(m) => m.invertible(),
            };
...
pub fn form_vec_2x2(v: &Vec<Vec<String>>) -> Matrix2x2 {
...
pub fn form_vec_3x3(v: &Vec<Vec<String>>) -> Matrix3x3 {
...
pub fn form_vec_4x4(v: &Vec<Vec<String>>) -> Matrix4x4 {
