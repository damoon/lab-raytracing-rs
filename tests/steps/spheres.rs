...
/*
    ...
    steps.when("s.material ← m", |mut world, _ctx| {
        let mut obj = world.objects.get_mut("s").unwrap().deref().deref().clone();
        obj.material = world.m.clone();
        world.objects.insert("s".to_string(), Arc::new(obj));
        world
    });
    ...
    steps.given_regex(
        r#"^set_transform\((s|object|shape|s2|s3), (scaling|translation)\(([-0-9.]+), ([-0-9.]+), ([-0-9.]+)\)\)$"#,
        |mut world, ctx| {
    ...
    steps.when_regex(
        ...
        |mut world, ctx| {
            ...
            world
                .objects
                .insert(ctx.matches[1].to_string(), Arc::new(obj));
            world
        },
    );

    steps.then_regex(r#"^s.transform = (identity_matrix|t)$"#, |world, ctx| {
        let lookup = world.objects.get("s").unwrap().transform();
