...
pub fn steps() -> Steps<MyWorld> {
    ...
    steps.given_regex(r#"(\w+) ← color\(([-0-9.]+), ([-0-9.]+), ([-0-9.]+)\)$"#, |mut world, ctx| {
    ...
    steps.then_regex(r#"^(\w+) = point\(([-0-9.]+), ([-0-9.]+), ([-0-9.]+)\)$"#, |world, ctx| {
        let name = ctx.matches[1].clone();
        let r =  ctx.matches[2].parse::<f32>().unwrap();
        let g =  ctx.matches[3].parse::<f32>().unwrap();
        let b =  ctx.matches[4].parse::<f32>().unwrap();
    ...
    steps.then_regex(r#"^(\w+) \+ (\w+) = color\(([-0-9.]+), ([-0-9.]+), ([-0-9.]+)\)$"#, |world, ctx| {
    ...
    steps.then_regex(r#"^(\w+) \* ([-0-9.]+) = color\(([-0-9.]+), ([-0-9.]+), ([-0-9.]+)\)$"#, |world, ctx| {
        let name = ctx.matches[1].clone();
